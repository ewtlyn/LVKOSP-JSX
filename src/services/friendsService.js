...
export class FriendsService {
  ...
  async getPendingRequests(userId) {
    try {
      const { data, error } = await supabase
        .from('friendships')
        .select(`user_id, created_at, requester:profiles!friendships_user_id_fkey(id, username, name, avatar_url, bio)`)
        .eq('friend_id', userId)
        .eq('status', 'pending')
  ...
  async acceptFriendRequest(currentUserId, requesterId) {
    try {
      const { error: upErr } = await supabase.from('friendships')
      ...
      return { success: false, error: e?.message || 'Failed to accept request' }
    }
  }
