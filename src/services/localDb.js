...
export function findSession(userId, token) {
  ...
  const session = db.sessions.find(
    (s) => s.user_id === userId && s.token === token,
  );
...
export function addFriend(userId, friendId) {
  ...
  const reverseExists = db.friendships.some(
    (row) => row.user_id === friendId && row.friend_id === userId,
  );
  if (!reverseExists) {
    db.friendships.push({
      id: createId(),
  ...
  const chats = memberRows
    ...
    .map((chat) => {
      ...
      const unreadCount = db.messages.filter(
        (msg) =>
          msg.chat_id === chat.id && !msg.read && msg.sender_id !== userId,
      ).length;
      return {
        id: chat.id,
...
export function createChat(userId, friendId) {
  ...
  const userChats = db.chat_members
    ...
    .map((row) => row.chat_id);
  const friendChats = db.chat_members
...
export function getMessages(chatId) {
  ...
  const messages = db.messages
    ...
    .map((msg) => ({
      ...clone(msg),
      sender: getPublicProfile(getUserById(msg.sender_id)) || {
        id: msg.sender_id,
        username: "unknown",
        name: "Unknown",
        avatar_url: "",
  ...
  if (chat) {
    ...
    chat.last_message_content = lastMessage
      ? lastMessage.type === "image"
        ...
        : lastMessage.content
      : "";
    chat.last_message_at = lastMessage
...
export function toggleLike(postId, userId) {
  const db = loadDb();
  const existing = db.post_likes.find(
    (row) => row.post_id === postId && row.user_id === userId,
  );
  if (existing) {
    db.post_likes = db.post_likes.filter(
      (row) => !(row.post_id === postId && row.user_id === userId),
    );
    saveDb(db);
