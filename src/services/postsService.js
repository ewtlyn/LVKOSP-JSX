...
export class PostsService {
  ...
  async createPost(
    ...
    if (wallOwnerId && wallOwnerId !== authorId)
      row.wall_owner_id = wallOwnerId;
    ...
    if (extraFiles?.length) {
      try {
        const extraUrls = await Promise.all(
          extraFiles.map((f, i) => this.uploadPostImage(f, authorId)),
        );
        await supabase
          .from("post_media")
          .insert(
  ...
  async getComments(postId) {
    const { data, error } = await supabase
      .from("post_comments")
      .select(
        `id, post_id, user_id, content, created_at, user:profiles!post_comments_user_id_fkey(id, username, name, avatar_url)`,
      )
      .eq("post_id", postId)
  ...
  async getRepostOf(postId) {
    const { data } = await supabase
      .from("posts")
      .select(
        `id, content, media_url, created_at, author:profiles!posts_author_id_fkey(id, username, name, avatar_url)`,
      )
      .eq("id", postId)
      .maybeSingle();
    return data || null;
